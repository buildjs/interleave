        ...
        function(err) {
            if (! err) {
                interleaver.process(writeables, function() {
                    ...
                    combiner(interleaver, writeables, function(files) {
                        ...
                        interleaver.write(files, function(outputFiles) {
                            ...
                            postprocessors.run(interleaver, outputFiles, opts.after, function() {
                                interleaver.done();
                            });
                        });
                    });
...
### _loadConfig
*/    
function _loadConfig(interleaver, opts, callback) {
    ...
    // otherwise attempt to load a configuration file
        ...
        // get the target path
        out('!{grey}looking for optional build configuration file: ' + interleaver.configFile);
            ...
            if (exists) {
                fs.readFile(interleaver.configFile, 'utf8', function(err, data) {
                    try {
                        ...
                        out('!{red}Error loading build.json configuration, default config used');
                        callback();
                    } // try..catch
                });
            }
...
function Interleaver(opts) {
    this.targetPath = path.resolve(opts.path || 'dist');
...
exports = module.exports = function(targetFiles, opts) {
    ...
    // load the config and expand the paths
    _loadConfig(interleaver, opts, function() {
