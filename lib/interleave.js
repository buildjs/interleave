...
function _compile(interleaver, files, opts) {
    ...
    async.forEachSeries(
        ...
        function(file, callback) {
            ...
            fs.readFile(path.resolve(interleaver.basedir, file), 'utf8', function(err, fileContents) {
                if (err) {
                    out('!{bold}error:!{red}       ' + err);
                }
                else {
                    // set the rigger cwd
                    opts.cwd = path.dirname(path.resolve(interleaver.basedir, file));
                    
                    rigger.process(fileContents, opts, function(err, source) {
                        var sourceData;
...
Interleaver.prototype._createPackages = function(files, packageType, callback) {
    // initialise the package directory
    var interleaver = this,
        packagePath = path.join(this.targetPath, 'pkg', packageType),
        ...
        if (! err) {
            ...
            async.forEach(
                ...
                function(fileData, itemCallback) {
                    ...
                    fileData.module = path.basename(fileData.file, '.js');

                    // package the file
                    packager.call(
                        interleaver,
                        path.join(packagePath, fileData.file),
                        fileData,
                        itemCallback
                    );
                },
...
Interleaver.prototype._extractDeps = function(content, deps) {
    var output = [];
    ...
    content.split('\n').forEach(function(line) {
        ...
        if (match) {
            (match[1] || '').split(/\,\s*/).forEach(function(dep) {
                // add the dependency if not already counted
...
Interleaver.prototype.exportFiles = function(files) {
    var interleaver = this;
    ...
    if (this.targetPackages.length > 0) {
        async.forEach(
            this.targetPackages,
            this._createPackages.bind(this, files),
            function(err) {
