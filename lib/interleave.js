...
Interleaver.prototype.include = function(current, target, callback) {
    ...
    target = targetParts[0] + (supportedExtensions.indexOf(targetExt) >= 0 ? '' : '.js') + 
    ...
    console.log('include: '.grey + target);
...
Interleaver.prototype.write = function(files, callback) {
    ...
    files.forEach(function(fileData) {
        ...
        console.log('write:   '.magenta + targetFile);
        ...
        interleaver.expandPaths(targetFiles, function(err, files) {
            ...
            async.forEachSeries(
                ...
                function(file, callback) {
                    interleaver.include(null, path.resolve(file), function(err, sourceData) {
